...
def _build_response_policy(state: AgentState, schema: str = "") -> str:
    ...
    return (
        f"{role_prompt}\n\n"
        "Rules:\n"
        "1) Check already-collected data before calling a tool.\n"
        "2) NEVER invent SQL rows, numbers, or document citations.\n"
        "3) Use only exact table/column names from the NGA schema.\n"
        "4) For safety decisions, always cite the specific SOP/document ID and clause.\n"
        "5) When the answer is complete, return a JSON object with: "
